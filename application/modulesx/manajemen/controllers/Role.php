...
class Role extends Userauth {
    ...
    public function do_upd()
	{
        if($this->input->method() == 'post'){
            ...
            $nm_role=$this->input->post('nm_role');
            $data=array('nama_role'=>$nm_role);
            if($this->M_adm_role->get_upd($id,$data)){
                //$this->session->set_flashdata('notif','<div class="alert alert-success"><span class="glyphicon glyphicon-info-sign"></span> OK !!! data role berhasil diupdate..</div>');
