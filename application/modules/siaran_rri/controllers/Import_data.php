...
class Import_data extends Userauth {
    ...
    private $filenamexl='template-siaran_rri.xls';
    private $mulai_rowxl=8;
    
    function __construct(){
	   parent::__construct();
       $this->load->model('M_siaran_rri');
    ...
    public function index(){
        $data['total_data']=$this->M_siaran_rri->get_total();
        $data['total_import']=$this->M_siaran_rri->get_total('1');
            ...
            if($excel['name'] == $this->filenamexl){
                ...
                {   
                    ...
                    for($mulai=$this->mulai_rowxl; $mulai<=count($cells); $mulai++){
                        ...
                        $dt=array(
                            'judul_naskah'=>$cells[$mulai]['B'],
                            'penulis_naskah'=>$cells[$mulai]['C'],
                            'tgl_pelaksanaan'=>$tgl_pelaksanaan,
                            'tgl_perekaman'=>$this->set_date($cells[$mulai]['D']),
                            'tgl_disiarkan'=>$this->set_date($cells[$mulai]['E']),
                            'stts_import'=>1,
                            'user_input'=>$this->session->username
                        );
                        if(empty($cells[$mulai]['B']) && empty($cells[$mulai]['C']) && empty($cells[$mulai]['D']) && empty($cells[$mulai]['E'])){
                            //$this->session->set_flashdata('notif',alert('danger','Upss !!! data kosong...'));
                        }else{
                            $total[]=$dt;
                            $this->db->insert('t_siaran_rri',$dt);
                            $dt_lap=array(
