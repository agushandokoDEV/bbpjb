...
class Users extends Userauth {
		...
		if ($this->input->is_ajax_request()){
           $list = $this->M_adm_users->get_datatables();
            ...
            foreach ($list as $val) {
                ...
                if($val->status == '0'){
                    $stts='<span class="glyphicon glyphicon-question-sign bg-bulet bg-warning"></span> Belum aktivasi';
                }else if($val->status == '1'){
                    $stts='<i class="fa fa-check bg-bulet bg-success"></i> Aktif';
                }else if($val->status == '2'){
                    $stts='<span class="glyphicon glyphicon-remove bg-bulet bg-danger"></span> Non aktif';
                }
                $row[] = $stts;
                $ac="'".$val->username."'";
                $row[] = '
    ...
    public function do_upd()
	{
        if($this->input->method() == 'post'){
            ...
            $this->M_adm_users->get_upd($username,$data);
            $this->session->set_flashdata('notif',alert('success','OK !!! data users '.$nm_lengkap.' berhasil diupdate..'));
            redirect('manajemen/users');
        }else{
            show_404();
        }
