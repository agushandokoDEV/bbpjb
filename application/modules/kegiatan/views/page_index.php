        ...
        <div class="row">
            <div class="col-md-8">
                <ol class="breadcrumb">
                  ...
                  <li class="active">Kegiatan Lainnya</li>
                </ol>
            </div>
            <div class="col-md-4">
                <div class="pull-right">
                    <a href="<?php echo site_url('common/excel/template-kegiatan_lain.xls') ?>" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Template Ms.excel</a>
                    <a href="<?php echo site_url('kegiatan/import_data') ?>" class="btn btn-sm btn-info"><i class="fa fa-cloud-upload"></i> Import</a>
                    <a href="<?php echo site_url('kegiatan/add') ?>" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-plus"></span> Tambah Data</a>
                </div>
            </div>
        </div>
        <hr class="hr"/>
...
function hapus(id){
    $.confirm({
        ...
        confirm: function(){
            $.post('<?php echo site_url('kegiatan/hapus') ?>',{key:id},function(r){
...
function verify(id){
    ...
    $.post('<?php echo site_url('kegiatan/ajax_load_v') ?>',{key:id},function(r){
        $('#load-v').html(r);
    });
}
function do_verify(stts){
    ...
    $.post('<?php echo site_url('kegiatan/do_verify') ?>',{key:$('#id_key').val(),stts:stts},function(r){
        var n=jQuery.parseJSON(r);
        if(n.status == true){
            $('#mdl-v').modal('hide');
            table.ajax.reload(null,false);
