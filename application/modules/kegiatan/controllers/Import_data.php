...
class Import_data extends Userauth {
    ...
    private $filenamexl='template-kegiatan_lain.xls';
    ...
    public function index(){
        $data['total_data']=$this->M_kegiatan->get_total();
        $data['total_import']=$this->M_kegiatan->get_total('1');
                ...
                {   
                    ...
                    for($mulai=$this->mulai_rowxl; $mulai<=count($cells); $mulai++){
                        ...
                        $dt=array(
                            ...
                            'tahun'=>$tahun,
                            'nama_keg'=>$cells[$mulai]['C'],
                            'tgl_keg'=>$tgl_pelaksanaan,
                            'tgl_pelaksanaan'=>$tgl_pelaksanaan,
                            'tempat'=>$cells[$mulai]['E'],
                            'sasaran'=>$cells[$mulai]['F'],
                            'jum_peserta'=>$cells[$mulai]['G'],
                            'jum_penyuluh'=>$cells[$mulai]['H'],
                            'nama_penyuluh'=>$cells[$mulai]['I'],
                            'jum_peserta_penyuluh'=>$cells[$mulai]['J'],
                            'stts_import'=>1,
                            'user_input'=>$this->session->username
                        );
                        if(empty($cells[$mulai]['B']) && empty($cells[$mulai]['C']) && empty($cells[$mulai]['D']) && empty($cells[$mulai]['E']) && empty($cells[$mulai]['F']) && empty($cells[$mulai]['G']) && empty($cells[$mulai]['H']) && empty($cells[$mulai]['I']) && empty($cells[$mulai]['J'])){
                            //$this->session->set_flashdata('notif',alert('danger','Upss !!! data kosong...'));
                        }else{
                            $total[]=$dt;
                            $this->db->insert('t_kegiatan',$dt);
                            $dt_lap=array(
