...
class M_sosial_p_bhs_negara extends CI_Model{
    ...
    private $column=array('id_sosial_p_bhs_negara','nama_kota','judul','waktu','ranah','tmpt_sosialisasi','peserta_sosialisasi','panitia_daerah','nara_sumber');
    ...
    private function _get_datatables_query()
	{
        ...
        $this->db->where('t_sosial_p_bhs_negara.user_input',$this->session->username);
		$i = 0;
    ...
    private function get_curent_import(){
        $this->db->select('id_sosial_p_bhs_negara');
        $this->db->where("DATE_FORMAT(tgl_input,'%Y-%m')",date('Y-m'));
        $this->db->where('user_input',$this->session->username);
    ...
    private function del_import_lap(){
        ...
        foreach($id_key as $id_key){
            $in[]=$id_key->id_sosial_p_bhs_negara;
        }
