...
class Import_data extends Userauth {
    ...
    private $filenamexl='template-terbitan.xls';
    ...
    public function index(){
        $data['total_data']=$this->M_terbitan->get_total();
        $data['total_import']=$this->M_terbitan->get_total('1');
            ...
            if($excel['name'] == $this->filenamexl){
                ...
                {   
                    ...
                    for($mulai=$this->mulai_rowxl; $mulai<=count($cells); $mulai++){
                        $ar[]=$cells[$mulai];
                        $thn=$cells[$mulai]['I'];
                        $bln=get_bln($cells[$mulai]['H']);
                        $tgl_pelaksanaan=$thn.'-'.$bln.'-'.date('d');
                        
                        $dt=array(
                            'kat_terbitan'=>$cells[$mulai]['B'],
                            'penulis'=>$cells[$mulai]['C'],
                            'no_isbn'=>$cells[$mulai]['D'],
                            'thn_terbit'=>$cells[$mulai]['E'],
                            'info_produk'=>$cells[$mulai]['F'],
                            'deskripsi'=>$cells[$mulai]['G'],
                            'stts_import'=>1,
                            'thn_pelaksanaan'=>$thn,
                            'bln_pelaksanaan'=>$bln,
