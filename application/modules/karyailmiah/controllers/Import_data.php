...
class Import_data extends Userauth {
    ...
    private $filenamexl='template-karyailmiah.xls';
    ...
    public function index(){
        $data['total_data']=$this->M_karyailmiah->get_total();
        $data['total_import']=$this->M_karyailmiah->get_total('1');
                ...
                {   
                    ...
                    for($mulai=$this->mulai_rowxl; $mulai<=count($cells); $mulai++){
                        ...
                        $dt=array(
                            ...
                            'judul'=>$cells[$mulai]['B'],
                            'jenis'=>$cells[$mulai]['C'],
                            'penulis'=>$cells[$mulai]['D'],
                            'prodi'=>$cells[$mulai]['E'],
                            'perguruan_t'=>$cells[$mulai]['F'],
                            'tahun'=>$thn,
                            'bln_pelaksanaan'=>$bln,
                            //'tgl_pelaksanaan'=>$tgl_pelaksanaan,
                            'abstraksi'=>$cells[$mulai]['I'],
                            'stts_import'=>1,
                        ...
                        if(empty($cells[$mulai]['B']) && empty($cells[$mulai]['C']) && empty($cells[$mulai]['D']) && empty($cells[$mulai]['E']) && empty($cells[$mulai]['F']) && empty($cells[$mulai]['G'])){
                            ...
                            $total[]=$dt;
                            $this->db->insert('t_karyailmiah',$dt);
                            $dt_lap=array(
