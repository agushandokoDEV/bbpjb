...
class Import_data extends Userauth {
    ...
    private $filenamexl='template-penyusunan_bahan_ajar.xls';
    private $mulai_rowxl=8;
    
    function __construct(){
	   parent::__construct();
       $this->load->model('M_bahan_ajar');
    ...
    public function index(){
        $data['total_data']=$this->M_bahan_ajar->get_total();
        $data['total_import']=$this->M_bahan_ajar->get_total('1');
                ...
                {   
                    ...
                    for($mulai=$this->mulai_rowxl; $mulai<=count($cells); $mulai++){
                        ...
                        $dt=array(
                            ...
                            'tahun'=>$tahun,
                            'judul'=>$cells[$mulai]['D'],
                            'nama_penyusun'=>$cells[$mulai]['E'],
                            'kat'=>$cells[$mulai]['F'],
                            'thn_penyusun'=>$cells[$mulai]['G'],
                            'tingkat'=>$cells[$mulai]['H'],
                            'sasaran'=>$cells[$mulai]['I'],
                            'tema'=>$cells[$mulai]['J'],
                            'sumber_bahan'=>$cells[$mulai]['K'],
                            'stts_import'=>1,
                        ...
                        if(empty($cells[$mulai]['B']) && empty($cells[$mulai]['C']) && empty($cells[$mulai]['D']) && empty($cells[$mulai]['E']) && empty($cells[$mulai]['F']) && empty($cells[$mulai]['G']) && empty($cells[$mulai]['H'])&& empty($cells[$mulai]['I']) && empty($cells[$mulai]['J']) && empty($cells[$mulai]['K'])){
                            ...
                            $dt_lap=array(
                                ...
                                'tbl_data'=>'bahan_ajar',
                                'menu'=>'Penyusunan Bahan Ajar'
                            );
                            $this->db->insert('t_laporan',$dt_lap);
                        }
                    }
                    $this->session->set_flashdata('notif','<hr class="hr"/>'.alert('success','OK !!! Import data berhasil, nama file yang di import : '.$excel['name']));
