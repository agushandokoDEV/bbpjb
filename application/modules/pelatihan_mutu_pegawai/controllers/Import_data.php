...
class Import_data extends Userauth {
    ...
    private $filenamexl='template-pelatihan.xls';
    private $mulai_rowxl=8;
    
    function __construct(){
	   parent::__construct();
       $this->load->model('M_pelatihan_mutu_pegawai');
    ...
    public function index(){
        $data['total_data']=$this->M_pelatihan_mutu_pegawai->get_total();
        $data['total_import']=$this->M_pelatihan_mutu_pegawai->get_total('1');
    ...
    public function del_import(){
        if($this->input->method() == "post"){
            $this->M_pelatihan_mutu_pegawai->del_import();
            $this->session->set_flashdata('notif_del',alert('info','OK !!! Reset data berhasil...'));
            redirect('pelatihan_mutu_pegawai/import_data/');
        }else{
            show_404();
        }
    }
                ...
                {   
                    ...
                    for($mulai=$this->mulai_rowxl; $mulai<=count($cells); $mulai++){
                        ...
                        $dt=array(
                            ...
                            'tahun'=>$tahun,
                            'nm_pelatihan'=>$cells[$mulai]['C'],
                            'tgl_latihan'=>$tgl_pelaksanaan,
                            'tgl_pelaksanaan'=>$tgl_pelaksanaan,
                            'nm_lembaga'=>$cells[$mulai]['E'],
                            'jum_peserta'=>$cells[$mulai]['F'],
                            'stts_import'=>1,
                        ...
                        if(empty($cells[$mulai]['B']) && empty($cells[$mulai]['C']) && empty($cells[$mulai]['D']) && empty($cells[$mulai]['E']) && empty($cells[$mulai]['F'])){
                            ...
                            $total[]=$dt;
                            $this->db->insert('t_pelatihan_mutu',$dt);
                            $dt_lap=array(
                                'username'=>$this->session->username,
                                'tgl_input'=>$tgl_pelaksanaan,
                                'jenis'=>'pembinaan',
                                'id_pk'=>$this->db->insert_id(),
                                'tbl_data'=>'t_pelatihan_mutu',
                                'menu'=>'Pelatihan dan Peningkatan Mutu Pegawai'
                            );
