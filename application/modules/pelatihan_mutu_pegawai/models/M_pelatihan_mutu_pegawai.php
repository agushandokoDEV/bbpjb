...
class M_pelatihan_mutu_pegawai extends CI_Model{
    
    private $table = "t_pelatihan_mutu";
    private $column=array('id_pelatihan_mutu','nama_kota','tgl_latihan','nm_pelatihan','nm_lembaga','jum_peserta');
    private $order = array('nm_pelatihan' => 'ASC');
    private $pk='id_pelatihan_mutu';
    ...
    private function _get_datatables_query()
	{
        ...
        $this->db->join('t_kabkot','t_kabkot.id_kabkot=t_pelatihan_mutu.id_kabkot','LEFT');
        $this->db->where('t_pelatihan_mutu.user_input',$this->session->username);
		$i = 0;
    ...
    private function get_curent_import(){
        $this->db->select('id_pelatihan_mutu');
        $this->db->where("DATE_FORMAT(tgl_input,'%Y-%m')",date('Y-m'));
        $this->db->where('user_input',$this->session->username);
