...
class M_kamus extends CI_Model{
    ...
    private $table = "t_kamus";
    private $column=array('id_kamus','nama_kota','tgl_pelaksanaan','judul','penyusun','jenis','sasaran');
    private $order = array('jenis' => 'ASC');
    private $pk='id_kamus';
    ...
    private function _get_datatables_query()
	{
        ...
        $this->db->join('t_kabkot','t_kabkot.id_kabkot=t_kamus.id_kabkot','LEFT');
        $this->db->where('t_kamus.user_input',$this->session->username);
		$i = 0;
    ...
    // delte row untuk lap grafik
    function del_user_input($id_pk){
        $this->db->where('username', $this->session->userdata('username'));
        $this->db->where('id_pk',$id_pk);
        $this->db->where('tbl_data',$this->table);
        $this->db->delete('t_laporan');
