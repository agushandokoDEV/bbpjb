...
class Import_data extends Userauth {
    ...
    private $filenamexl='template-penyusunan_ensiklopedi.xls';
    private $mulai_rowxl=11;
    ...
    public function index(){
        $data['total_data']=$this->M_ensiklopedi->get_total();
        $data['total_import']=$this->M_ensiklopedi->get_total('1');
                ...
                {   
                    ...
                    for($mulai=$this->mulai_rowxl; $mulai<=count($cells); $mulai++){
                        ...
                        $dt=array(
                            ...
                            'judul'=>$cells[$mulai]['D'],
                            'kat'=>$cells[$mulai]['E'],
                            'tim_redaksi'=>$cells[$mulai]['F'],
                            //'edisi'=>$cells[$mulai]['G'],
                            //'no_isbn'=>$cells[$mulai]['H'],
                            'lingkup'=>$cells[$mulai]['G'],
                            //'penerbit'=>$cells[$mulai]['J'],
                            'thn_terbit'=>$cells[$mulai]['H'],
                        ...
                        if(empty($cells[$mulai]['B']) && empty($cells[$mulai]['C']) && empty($cells[$mulai]['D']) && empty($cells[$mulai]['E']) && empty($cells[$mulai]['F']) && empty($cells[$mulai]['G']) && empty($cells[$mulai]['H']) && empty($cells[$mulai]['I']) && empty($cells[$mulai]['J'])){
                            ...
                            $total[]=$dt;
                            $this->db->insert('t_ensiklopedi',$dt);
                            $dt_lap=array(
